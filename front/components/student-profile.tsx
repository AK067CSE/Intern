...
function fillMissingDays(submissions: { date: string; count: number }[], days = 365) {
  ...
  const submissionMap = Object.fromEntries(submissions.map(s => [s.date, s.count]));
  return allDays.map(date => ({
    date,
    count: submissionMap[date] || 0,
  }));
}

function groupByWeeks(days: { date: string; count: number }[]) {
  const weeks = [];
        ...
        <Tabs defaultValue="contests" className="mt-6">
          ...
          <TabsContent value="problems" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                {(() => {
                  const days = fillMissingDays(problemStats?.submissions || [], 365);
